...
export class World {
  constructor(canvas) {
    ...
    this.playerMachines={street:createBike(shadow),scooter:createScooter(shadow),supersport:createSuperSport(shadow),horse:createHorse(shadow),robovac:createRobotVacuum(shadow)};
  ...
  setMachine(id) {
    const next=this.playerMachines[id]??this.playerMachines.street;
  ...
  render(sim, settings, frameDt=0) {
    ...
    for(const brush of damage.brushes??[])brush.rotation.y=sim.distance*4+brush.userData.phase;
