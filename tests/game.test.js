...
test('playables adapter loads before cloud save and uses YouTube ads',async()=>{
  ...
  const sdk={IN_PLAYABLES_ENV:true,
    game:{loadData:async()=>{calls.push('load');return JSON.stringify({schemaVersion:2,bestScore:80,bestDistance:40,bestCombo:3,tutorialCompleted:true,selectedMachine:'street',settings:{sfx:true,haptics:true}});},saveData:async data=>{calls.push(['save',JSON.parse(data).bestScore]);},firstFrameReady:()=>calls.push('first'),gameReady:()=>calls.push('ready')},
