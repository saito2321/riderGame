...
test('local save persists current settings and reloads best records',()=>{
  ...
  p.record({score:123.9,distance:50.8,bestCombo:4});p.setSetting('sfx',false);p.setSetting('haptics',false);p.completeTutorial();p.save(true);
  const reload=new LocalAdapter(storage).load();assert.equal(reload.bestScore,123);assert.equal(reload.bestDistance,50);assert.deepEqual(reload.settings,{sfx:false,haptics:false});assert.equal(reload.tutorialCompleted,true);assert.equal(reload.selectedMachine,'street');
});
...
test('local rewarded unlock persists only a valid machine and permits selecting it',async()=>{
  ...
  assert.equal(p.setMachine('horse'),false);
  const reward=p.requestMachineUnlock();assert.equal(p.requestMachineUnlock(),reward);
  p.resolveMachineUnlock(false);assert.equal(await reward,false);assert.equal(p.data.adUnlockedMachines.length,0);
  assert.equal(p.unlockMachine('missing'),false);
  assert.equal(p.unlockMachine('horse'),true);assert.equal(p.unlockMachine('horse'),false);
