...
ゲーム画面はアンチエイリアスを有効にし、初期DPRを端末DPRと1.5の小さい方に設定します。55fps以上が続くとDPRを最大2.0まで段階的に上げ、45fps未満が続くと最小0.75まで下げます。端末のCPUコア数・メモリーのブラウザー報告値は遠方車両の詳細表示にも使います。これらの報告値は端末性能の確定判定ではありません。実際の低価格Android/iPhoneでのFPS・発熱・メモリーの確認は別途必要です。
