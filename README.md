...
Best Score、Best Distance、Best Combo、チュートリアル完了、選択マシン、効果音と振動の設定を `lsr.save.v2` に保存します。公開前のため旧形式からの移行処理は持ちません。不正データは上書きせずセッション内でプレイできます。保存制限や失敗時は結果画面に表示します。
